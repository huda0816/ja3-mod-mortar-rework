function HUDAPrintAttacker(unit) 
    -- print("Attackerteam", unit.team.player_team)
end

function HUDACloseAllDialogs()
...
OnMsg.SelectedObjChange = HUDACloseAllDialogs
OnMsg.UnitSwappedWeapon = HUDACloseAllDialogs
...
function HUDABombardMishapChance(attacker, target, async)
    ...
    HUDAPrintAttacker(new_attacker)
...
function HUDAGetMaxAttacks(self, unit)
    ...
    local shots = Min(3, Min(weapon.ammo.Amount, round(aps / adjusted_shot_setup, 1)))
...
function HUDABombardAccuracyCheck(attacker, radius, i, num_shots, pos)
    ...
    local angle_part = 21000 / num_shots;
