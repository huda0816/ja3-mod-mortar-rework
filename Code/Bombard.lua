DefineClass.Ordnance = {
    __parents = {"SquadBagItem", "OrdnanceProperties", "InventoryStack"}
}
MapVar("g_Bombard", {})
PersistableGlobals.g_Bombard = false
MapVar("bombard_activate_thread", false)
PersistableGlobals.bombard_activate_thread = false
function _ENV:ExplosionPrecalcDamageAndStatusEffects(attacker, target, attack_pos, damage, hit, effect, attack_args,
    record_breakdown, action, prediction)
    ...
    BaseWeapon.PrecalcDamageAndStatusEffects(self, attacker, target, attack_pos, damage, hit, effect, attack_args,
        record_breakdown, action, prediction)
...
local ExplorationBombardUpdate = function()
    ...
    for idx, zone in ipairs(g_Bombard) do
        if zone.attacker and zone.attacker.combat_behavior ~= "PreparedBombardIdle" and zone.attacker.combat_behavior ~=
            "PrepareBombard" then
...
function BombardZone:Setup(pos, radius, side, ordnance, num_shots, activation_time)
    ...
    if activation_time then
        ...
        self.timer_text = CreateBadgeFromPreset("InteractableBadge", {
            target = self,
            spot = "Origin"
        })
...
function BombardEffect:__exec()
    ...
    g_Combat:QueueBombard(self.BombardId, team, self.AreaRadius, self.Ordnance, self.NumShots, self.LaunchOffset,
        self.LaunchAngle)
end
